...
async function getMocksFromPlugins(logger, plugins = []) {
  const mocks = await Promise.all(
    plugins.map(({ name, handler }) => Promise.resolve(handler?.({ logger: logger(name) }))),
  );
...
async function buildContent(contents) {
  ...
  // @ts-ignore
  const { outputFiles } = await esbuild.build(esbuildOptions);
