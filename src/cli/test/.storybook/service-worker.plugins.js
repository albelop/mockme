...
var Matcher = class {
  ...
  #findMock({
    ...
    if (mock) {
      ...
      const result = Response.json(body2, {
        status,
  ...
  set scenario(scenario) {
    if (this.#scenarios.includes(scenario) || scenario === void 0) {
...
function matchConditions({ header, cookie, body, query, url }, request) {
  return (request.conditions?.url ? compareObjects(url, request.conditions?.url) : true) && compareObjects(header, request.conditions?.header) && compareObjects(cookie, request.conditions?.cookie) && compareObjects(body, request.conditions?.body) && compareObjects(query, request.conditions?.query);
}
...
function filterEmptyOptions(obj = {}) {
  return Object.keys(obj).reduce((result, key) => {
    if (Array.isArray(obj[key]) && obj[key].length !== 0) {
      ...
      result[key] = obj[key];
    } else if (typeof obj[key] !== "object" && (Boolean(obj[key]) || obj[key] === 0 || obj[key] === false)) {
      result[key] = obj[key];
    }
    return result;
  }, {});
...
var BROADCAST_CHANNEL = "mockme";
var GET_SCENARIOS_MESSAGE_TYPE = "get:scenarios";
var SET_SCENARIO_MESSAGE_TYPE = "set:scenario";
var SCENARIOS_MESSAGE_TYPE = "scenarios";
...
function serviceWorker(sw, mocks) {
  ...
  sw.addEventListener("fetch", (event) => {
    ...
    if (path.indexOf(".") === -1 && path !== "/") {
      const match2 = matcher.match(event.request);
      if (match2.response) {
        event.respondWith(match2.delayedResponse());
      }
    }
  });
